...
export const LINK_TXT_URL = 'https://mergvs.com/demo3-xlsx/link.txt';

// --- Scene start rows — dynamically discovered from xlsx ---
// No hardcoded rows. discoverSceneRows(rows) scans for "▌ SAHNE"
// markers and builds the map automatically.
...
export async function loadVeaConfig(xlsxPath) {
    ...
    const sceneStartRows = discoverSceneRows(rows);
    const scenes = {};
...
/**
 * Scan rows for "▌ SAHNE N" markers and return { N: rowIndex }.
 * Works with any number of scenes in any xlsx layout.
 */
function discoverSceneRows(rows) {
    const map = {};
    for (let i = 0; i < rows.length; i++) {
        ...
        if (m) {
            map[parseInt(m[1], 10)] = i + 1; // 1-indexed Excel row
        }
    }
    return map;
}
...
function parseScene(rows, sceneStartRowExcel, assets) {
    ...
    const sceneType = String(camera['Tip'] || 'orbit').toLowerCase()
        .replace(/\s*\(.*\)/, '').trim(); // strip "(FPS)" etc.
