...
// Resolved relative to this module, so the demo4 folder is fully
// self-contained and works on any host (mergvs.com, previews, local).
export const LINK_TXT_URL = new URL('./link.txt', import.meta.url).href;
...
export async function resolveXlsxUrl(linkTxtUrl = LINK_TXT_URL) {
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        // Relative entries (e.g. "demo4-input.xlsx") resolve against
        // link.txt's own location; absolute http(s) URLs pass through.
        return new URL(line, linkTxtUrl).href;
    }
